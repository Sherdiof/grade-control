...
<div class="p-2 w-full">
    <div class="relative">
        ...
        <input type="number" id="value" name="value" step="any" value="{{ $edit ? old('value', $homework->value) : old('value') }}"
               class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
        @error('value')
        <p class="text-red-800 my-1 rounded-lg text-sm px-3">{{ $message }}</p>
        @enderror
                ...
                @foreach($course as $c)
                    @if($edit)
                        @if($course->id == $homework->assigment->course->id)
                            <option selected
                                    value="{{ $homework->assigment_id }}">{{ $homework->assigment->course->name }}</option>
                        @else
